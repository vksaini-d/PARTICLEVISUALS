export const velocityFragmentShader = `
    ...
    // Standard Simplex Noise 3D
    // Copied from Ashima Arts / Stefan Gustavson. 
    // Don't touch the math unless you really, REALLY understand partial derivatives.
    ...
    void main() {
        ...
        // --- SHAPE DEFINITIONS ---
        // Warning: Some of these look like black magic because I tuned them by eye.
        
        if (shape == 22) { // HUMAN FACE (SDF Approximation)
            // It's not a real 3D model (too heavy), just a bunch of SDF spheres blended together.
            // Works well enough for a particle system.
            float r = rnd.x;
            if (r < 0.6) { // Head
                vec3 p = normalize(rnd - 0.5) * 80.0;
                p.y *= 1.3; // Basic Oval shape
                p.z *= 0.9;
                target = p;
            } else if (r < 0.8) { // Eyes
                ...
                // Sound: Eyes blink/pulse to the beat
                // Multiplier 10.0 was found by trial and error
                target.z += uHigh * 10.0;
            } else { // Mouth
                float t = (rnd.y - 0.5) * 2.0;
                target.x = t * 20.0;
                target.y = -30.0 + sin(t * 3.14) * 10.0; // Simple smile curve
                target.z = 70.0;
                
                // Sound: Mouth opens when mid-range freq hits
                target.y -= uMid * 20.0 * (1.0 - abs(t));
            }
            
            // Add some noise so it doesn't look too plastic
            ...
            // Radius expands with sound. Base 100 seems like a good size.
